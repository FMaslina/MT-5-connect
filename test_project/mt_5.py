...
MT2_PATH = r'C:\Program Files\MetaTrader 5 2\terminal64.exe'
MT1_LOGIN = 805060
MT2_LOGIN = 805061
MT1_PASS = 'bsccvba1'
MT2_PASS = 'weax5szp'
...
def checking_an_open_transaction(transaction_plus, transaction_minus, price_open, price_current, type_of_order,
    ...
    if type_of_order == 0:
        ...
        res = (price_current - price_open) / price_open * 100     # Расчет сделки продажи по формуле
    ...
    for pos_snd in positions_sender:
        ...
        for pos_rec in positions_receiver:
            ...
            try:
                ...
                continue
        if not skip_this:
            #   Проверка на отклонение перед открытием позиции
            if checking_an_open_transaction(transaction_plus=0.01, transaction_minus=-0.01,
                                            price_open=position_s['price_open'],
                                            price_current=position_s['price_current'],
                                            type_of_order=position_s['type']):
                ...
                print("The deviation is too large")
        else:
            tp_p = sl_p = 0
